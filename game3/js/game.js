...
function g() {
	...
	if(Number(r)>50){
		...
		if(sessionStorage.getItem('icepailpoint')!=null){
			var point=sessionStorage.getItem('icepailpoint');
			var totalpoint=Number(point)+300;
			sessionStorage.setItem('icepailpoint',totalpoint);
				
		}else{
			sessionStorage.setItem('icepailpoint','300');
		}
		
	}
	
	
	aa.innerHTML = 'You have successfully cleaned up data by <span style="color:#f00">' + r + "</span> times, earned <span style='color:#f00'>"+thispoint+"</span> points! Congratulations!" ;
	da(r);
	console.log(r,k,ca);
